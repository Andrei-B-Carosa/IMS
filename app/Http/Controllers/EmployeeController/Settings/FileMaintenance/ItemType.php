...
class ItemType extends Controller
{
    public function dt(Request $rq)
    {
        ...
        $data = ImsItemType::when($filter_status,function($q) use($filter_status){
    ...
    public function update(Request $rq)
    {
        try{
            ...
            $values = [
                ...
                'item_code' =>$rq->item_code,
                'item_number' =>$rq->item_number,
                'display_to' =>$rq->display_to,
            ];
            $query = ImsItemType::updateOrCreate($attribute,$values);
    ...
    public function delete(Request $rq)
    {
        try{
            ...
            return response()->json([
                'status' => 'info',
                'message'=>'Success',
                'payload' => ImsItemType::where('is_active',1)->count()
            ]);
        }catch(Exception $e){
            DB::rollback();
            return response()->json([
    ...
    public function validate(Request $rq)
    {
        try {
            ...
            $availableItems = ImsItemType::whereRaw("REPLACE(LOWER(name), ' ', '') = ?", [$normalizedName])
            ->where('is_active', 1)
            ->count();
            if ($availableItems > 0) {
                $valid = false;
