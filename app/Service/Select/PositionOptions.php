...
class PositionOptions
{
    ...
    {
        $query = HrisPosition::where('is_deleted',null)->when($rq->view!='all', fn($q) => $q->where('is_active', $rq->view));

        $search = isset($rq->id) ? Crypt::decrypt($rq->id) : false;
        $html = '<option></option>';

        return match($rq->type){
            'position' => $this->get_position($query,$search,$html),
        };
    }


    public function get_position($query,$search,$html)
    {
        $data = $query->get();
        if ($data->isEmpty()) {
