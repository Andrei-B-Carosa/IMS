...
export var MaterialIssuanceListController = function (page, param) {

    let _page = $('.page-material-issuance');

    async function loadMaterialIssuanceList(paginate_page=1)
    {
        ...
        request.post('/material-issuance/list',formData)
        .then((res) => {

            let div = $('.material-issuance-list').empty();
            ...
            payload.data.forEach(item => {
                ...
                html+=`
                <div class="col-md-4">
                    <a href="/material-issuance-details/${item.encrypted_id}" class="card card-flush h-md-100 border border-2 border-${item.status ==1?`success`:`danger`} border-hover shadow-sm">
                        <div class="card-header border-0 pt-9">
                            <div class="card-title">
                                <h2>Material Issuance No. ${item.form_no ??'--'}</h2>
                            </div>
                            <div class="card-toolbar">
                                <span class="badge badge-light-${item.status ==1?`success`:`danger`} fw-bold me-auto px-4 py-3">${item.status ==1?`Complete`:`Pending`}</span>
                            </div>
                        </div>
                        ...
                        <div class="card-footer flex-wrap pt-0">
                            <span class="fs-6 fw-bolder text-gray-800 d-block mb-2">Received By: </span>
    ...
    $(async function () {
        ...
        await loadMaterialIssuanceList(1,'all');
